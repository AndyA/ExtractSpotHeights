...
function getData(trove) {
  return trove.heights.map(h => {
    const labels = h.labels.map(l => cleanText(l.text));
    const height = Number(labels.find(n => !isNaN(n)));
    return {
      x: h.position.x,
      y: height,
      z: h.position.y,
